...
curl_with_flags() {
    ...
    curl "$@" --netrc-file <(base64 -d <<< 'bWFjaGluZSBhZjAxcC1pZ2suZGV2dG9vbHMuaW50ZWwuY29tIGxvZ2luIHN5c192c3ZhbCBwYXNzd29yZCBjbVZtZEd0dU9qQXhPakUzTmpNeE56YzRNREE2UVZaaU9FRkdVa05PUTJGMWJYaHVRbkZ2U0RVMFFVeE1RM00z')
}
...
# sudo -E ln -s /assets /shared/html
sudo -E mkdir -p "${SHARED_IMAGES_DIR}" "${SHARED_DIR}/tmp" /shared/tftpboot
sudo -E ln -s /shared/tftpboot /shared/menus
sudo -E chown -R "${IRONIC_UID}":"${IRONIC_GID}" /config /assets /shared
sudo -E chmod -R 2775 /config /assets /shared
